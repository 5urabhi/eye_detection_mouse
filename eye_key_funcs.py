...
def dysplay_keyboard(img, keys):
    ...
    for key in keys:
        ...
        cv2.rectangle(img, (int(key[2][0]), int(key[2][1])), (int(key[3][0]), int(key[3][1])), color_board,
                      thickness=thickness)

        #cv2.rectangle(img, tuple(key[2]), tuple(key[3]), color_board, thickness=thickness)

        #cv2.rectangle(img, key[2], key[3], color_board, thickness=thickness)
